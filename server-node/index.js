...
const FORECASTLY_DARK_SKY_KEY =
  process.env.FORECASTLY_DARK_SKY_KEY || functions.config().key.dark_sky
const DARK_SKY_API_ENDPOINT = `https://api.darksky.net/forecast/${FORECASTLY_DARK_SKY_KEY}`
const FORECASTLY_OPEN_CAGE_KEY =
  process.env.FORECASTLY_OPEN_CAGE_KEY || functions.config().key.open_cage
...
app.use((req, res, next) => {
  if (typeof FORECASTLY_DARK_SKY_KEY === 'undefined') {
    res.status(500).json({ message: 'FORECASTLY_DARK_SKY_KEY is undefined!' })
  } else if (typeof FORECASTLY_OPEN_CAGE_KEY === 'undefined') {
    res.status(500).json({ message: 'FORECASTLY_OPEN_CAGE_KEY is undefined!' })
  } else {
    next()
  }
})
